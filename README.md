...
Every plugin here is **pure ESM with no build step**, so no `prepare` script runs at install time and pnpm never asks you to allowlist a build. That allowance is permission to execute a package's code on your machine, and none of these packages need it.
...
- **No build step.** Pure ESM, so the harness's own cordis instance is shared rather than duplicated.
